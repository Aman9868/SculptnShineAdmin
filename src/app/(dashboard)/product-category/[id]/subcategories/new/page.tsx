...
export default function AddNewSubcategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.slug) {
      setError("Subcategory Name and Slug are required.");
      return;
    }
  ...
  return (
    <div className="max-w-5xl mx-auto animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex items-center gap-4 mb-8">
        <Link href={`/product-category/${categoryId}`} className="p-2 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
        ...
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
            ...
            <span>›</span>
            <Link href={`/product-category/${categoryId}`} className="hover:text-gold-600 truncate max-w-[150px]">{category?.name || 'Category'}</Link>
            <span>›</span>
            <span className="text-gray-900 font-semibold">Add Subcategory</span>
          </div>
          <h1 className="text-3xl font-extrabold text-brandDark font-serif-luxury tracking-tight">Add New Subcategory</h1>
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-6">
              {/* Parent Category Field (Read-only) */}
              <div className="space-y-2 opacity-70">
                ...
                <input
                  ...
                  value={category?.name || ''}
                  disabled
                  className="w-full px-4 py-2.5 bg-gray-100 border border-gray-200 rounded-xl text-sm font-semibold text-gray-600 cursor-not-allowed"
                />
              </div>

              {/* Subcategory Name */}
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700">Subcategory Name <span className="text-red-500">*</span></label>
                <input
                  type="text"
                ...
                />
                <p className="text-xs text-gray-500">e.g. Whey Protein, Pre-Workout, Dumbbells</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-gray-500">URL friendly unique identifier (e.g. whey-protein)</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  <p className="text-xs text-gray-500">Provide a brief overview of what this subcategory contains.</p>
          ...
          <div className="flex items-center justify-end gap-3 pt-8 mt-6 border-t border-gray-100">
            <button
              type="button"
              onClick={() => router.push(`/product-category/${categoryId}`)}
